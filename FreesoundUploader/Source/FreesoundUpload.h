...
public:
	...
	FreesoundRequest(URL url, String POSTData, String inAccessToken)
		...
		logger(File(FileLogger::getSystemLogFileFolder().getFullPathName() + File::getSeparatorChar() + "FSUPFRlog.txt"), "Freesound Uploader log file, FSR:\n")
	{
	}
	...
	Response makeRequest()
	{
		...
		//Logging functions
		logger.logMessage("Response Headers: " + headers);
		logger.logMessage("url = " + requestedCall.toString(true));
			...
			&responseHeaders, &statusCode)))
		{
			...
			logger.logMessage((statusCode != 0 ? "Status code: " + String(statusCode) + newLine : String())
...
public:
	...
	FreesoundAuthorization(String inClientID, String inClientSecret)
		...
		authCode(),
		refreshToken(),
		logger(File(FileLogger::getSystemLogFileFolder().getFullPathName() + File::getSeparatorChar() + "FSUPauthlog.txt"), "Freesound Uploader log file, FSA:\n")
	...
	int codeExchange() {
		...
		if (incoming.first == 200) {
			...
			logger.logMessage("Finished code exchange with acctoken, refreshtoken" + String(accessToken) + "+" + String(refreshToken));
		}

		logger.logMessage("codeXChange status= " + String(incoming.first));
...
private:
	...
	void pageFinishedLoading(const String &url) {
		...
		if (url.startsWith("https://freesound.org/home/app_permissions/permission_granted/")) {
			...
			logger.logMessage("Page finished loading: code: " + authCode);
			callBack();
		}
	}

	void pageLoadHadNetworkError() {
		logger.logMessage("Page load had network error");
		String loginURL = "https://freesound.org/apiv2/oauth2/authorize/?client_id=" + clientID + "&response_type=code";
		goToURL(loginURL);
	}
